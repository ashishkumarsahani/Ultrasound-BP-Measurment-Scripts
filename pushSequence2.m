...
number_Of_Angles = 7; %Always set to odd
...
Resource.RcvBuffer(1).rowsPerFrame = 2^nextpow2(2*round(ceil(2*(RF_Points_Per_Wavelength*(end_RF_Depth/wavelength_in_mm)))/2))* (number_Of_Angles+ num_of_Frames_Per_Push_Cycle-1); %range is set to double the target distance.
...
Receive = repmat(struct('Apod', zeros(1,Trans.numelements), ...
                        ...
                        'callMediaFunc', 0),1,(num_of_Frames_Per_Push_Cycle-1+number_Of_Angles)*length(voltage_Levels)*length(time_List));
                    
% - Set event specific Receive attributes.
for i = 1:(num_of_Frames_Per_Push_Cycle-1+number_Of_Angles)*length(voltage_Levels)*length(time_List)
    ...
    Receive(i).mode = 0;
    Receive(i).bufnum = 1;
    Receive(i).framenum = ceil(i/(num_of_Frames_Per_Push_Cycle-1 + number_Of_Angles));
    Receive(i).acqNum = mod(i-1,(num_of_Frames_Per_Push_Cycle-1 + number_Of_Angles)) +1;
...
ReconInfo = repmat(struct('mode', 4, ...  % default accumulate IQ data.
                   ...
                   'rcvnum', 1, ...
                   'pagenum',1,...
                   'regionnum', 0), 1, (num_of_Frames_Per_Push_Cycle-1+number_Of_Angles)*length(voltage_Levels)*length(time_List));

% - Set event specific Recon attributes.
%set first frame recon info

for j =0:(num_of_Frames_Per_Push_Cycle-1+number_Of_Angles):length(ReconInfo)-1
    ReconInfo(j+1).mode = 0;
    ReconInfo(j+1).rcvnum = j+1;
    ReconInfo(j+1).txnum = ceil(number_Of_Angles/2);
    
    %set second frame recon info (Multi flash angles)
    for i = 2:number_Of_Angles+1
        if(number_Of_Angles ==1)
            ReconInfo(j+i).mode = 0;
        elseif(i==2)
            ReconInfo(j+i).mode = 3; %replace IQ data
        elseif(i == number_Of_Angles+1)
            ReconInfo(j+i).mode = 5; %Detect
        else
            ReconInfo(j+i).mode = 4; %Accumulate
        end
        ReconInfo(j+i).rcvnum = j+i;
        ReconInfo(j+i).pagenum = 1;
        ReconInfo(j+i).txnum = i-1;
    end

    %Reconinfo for all other frames
    for i = number_Of_Angles+2 :number_Of_Angles + (num_of_Frames_Per_Push_Cycle-1)
        ReconInfo(j+i).mode = 0;
        ReconInfo(j+i).rcvnum = j+i;
        ReconInfo(j+i).pagenum = 1;
        ReconInfo(j+i).txnum = ceil(number_Of_Angles/2);
    end
end

count =0;
for j =0:num_of_Frames_Per_Push_Cycle:length(Recon)-1
    Recon(j+1).RINums = count*(num_of_Frames_Per_Push_Cycle-1+number_Of_Angles)+1; 
    Recon(j+2).RINums = count*(num_of_Frames_Per_Push_Cycle-1+number_Of_Angles)+(2:number_Of_Angles+1)';
    for i = 3:num_of_Frames_Per_Push_Cycle
        Recon(j+i).RINums = count*(num_of_Frames_Per_Push_Cycle-1+number_Of_Angles)+ (i-2)+number_Of_Angles+1; 
    end
    for i = 1:num_of_Frames_Per_Push_Cycle
        Recon(j+i).ImgBufDest = [1,j+i];
        Recon(j+i).IntBufDest = [1,j+i];
        if(i>1)
            Recon(j+i).newFrameTimeout= 1;
        end
    end
    count = count+1;
end
...
SeqControl(11).argument = time_to_Next_Tracking_Image;%(time_to_Next_Tracking_Image*1000)/200;  %value*200 ns

timing_List_in_us(1) = 0; % Time of reference frame
push_Timing_Start_End(1) = 0; %push delay after reference image microseconds 
push_Timing_Start_End(2) = push_Timing_Start_End(1) + time_List(end)*time_us_between_push - gap_Between_Pulses_in_us; %Total time of push pulses
...
Event(event_Count).rcv = 1 + (l-1)*(number_Of_Angles + (num_of_Frames_Per_Push_Cycle-1)); % use 1st Rcv structure.
Event(event_Count).recon = 0; % no reconstruction.
...
image_Pulse_Num = i;
Event(event_Count).info = 'Acquire the flash angle for high-resolution image';
Event(event_Count).tx = image_Pulse_Num; % use 1st TX structure.
Event(event_Count).rcv = i+1 + (l-1)*(number_Of_Angles + (num_of_Frames_Per_Push_Cycle-1)); % use 1st Rcv structure.
Event(event_Count).recon = 0; % no reconstruction.
Event(event_Count).process = 0; % read voltage here
Event(event_Count).seqControl = 0; %no delay between flash angles required here
event_Count = event_Count+1;
...
for i = 3:num_of_Frames_Per_Push_Cycle
    ...
    Event(event_Count).tx = image_Pulse_Num; % use 1st TX structure.
    Event(event_Count).rcv = (l-1)*(number_Of_Angles + (num_of_Frames_Per_Push_Cycle-1)) + number_Of_Angles+1+(i-2); % use 1st Rcv structure.
    Event(event_Count).recon = 0; % no reconstruction.
    Event(event_Count).process = 0; % no processing
    Event(event_Count).seqControl = [11]; % Wait noop between flash angle pushes
    event_Count = event_Count + 1;
end
Event(event_Count-1).seqControl = [seq_Count]; % Transfer to host
