...
number_Of_Angles = 1; %Always set to odd
...
Resource.RcvBuffer(1).rowsPerFrame = 2^nextpow2(2*round(ceil(2*(RF_Points_Per_Wavelength*(end_RF_Depth/wavelength_in_mm)))/2))*no_Frames_Per_Cardiac_Push; %range is set to double the target distance.
...
for i = 1:no_Frames_Per_Cardiac_Push*total_No_of_Pings
    ...
    Receive(i).mode = 0;
    Receive(i).bufnum = 1;
    Receive(i).framenum = ceil(i/no_Frames_Per_Cardiac_Push);
    Receive(i).acqNum = mod(i-1,no_Frames_Per_Cardiac_Push)+1;
...
Recon = repmat(struct('senscutoff', 0.6, ...
               ...
               'newFrameTimeout', -1,...
               'RINums', 1), 1, no_Frames_Per_Cardiac_Push*total_No_of_Pings);
...
ReconInfo = repmat(struct('mode', 0, ...  % default accumulate IQ data.
                   ...
                   'rcvnum', 1, ...
                   'pagenum',1,...
                   'regionnum', 0), 1, no_Frames_Per_Cardiac_Push*total_No_of_Pings);

% - Set event specific Recon attributes.
%set first frame recon info

for j =1:no_Frames_Per_Cardiac_Push*total_No_of_Pings
    ReconInfo(j).mode = 0;
    ReconInfo(j).rcvnum = j;
    Recon(j).RINums = j;
    Recon(j).ImgBufDest = [1,j];
end
...
seq_Count = 14; 
event_Count = 2;
packet_Time = reference_To_Push_Delay+time_us_between_push*(curr_No_of_Sub_Pulses-1) + time_us_pushend2imagestart+ (no_Frames_Per_Cardiac_Push-2)*time_to_Next_Tracking_Image;
...
Event(event_Count).process = 7; % read pressure here
Event(event_Count).seqControl = [seq_Count 2 6]; %Reference frame to push pulse delay
event_Count = event_Count+1;
...
% Event(event_Count).seqControl = [6 seq_Count]; %Reference frame to push pulse delay
% event_Count = event_Count+1;
% SeqControl(seq_Count).command = 'jump'; % jump back to start.
...
Event(event_Count).tx = image_Pulse_Num; % use 1st TX structure.
Event(event_Count).rcv = 1 + (l-1)*(no_Frames_Per_Cardiac_Push); % use 1st Rcv structure.
Event(event_Count).recon = 0; % no reconstruction.
Event(event_Count).process = 0; % read pressure here
Event(event_Count).seqControl = [11]; %Reference frame to push pulse delay
event_Count = event_Count+1;

timing_List_in_us(1 + (l-1)*(no_Frames_Per_Cardiac_Push)) = time_to_Next_Tracking_Image*((l-1)*(no_Frames_Per_Cardiac_Push))+(l-1)*round(1000000/pings_per_Second);
...
for i = 2:no_Frames_Per_Cardiac_Push
    timing_List_in_us((l-1)*(no_Frames_Per_Cardiac_Push)+ i) = timing_List_in_us((l-1)*(no_Frames_Per_Cardiac_Push)+ i-1)+time_to_Next_Tracking_Image;
    image_Pulse_Num = ceil(number_Of_Angles/2);
    Event(event_Count).info = ['RF Data frame ',num2str(i-2+1), 'after the push'];
    Event(event_Count).tx = image_Pulse_Num; % use 1st TX structure.
    Event(event_Count).rcv = (l-1)*(no_Frames_Per_Cardiac_Push)+ i; % use 1st Rcv structure.
    Event(event_Count).recon = 0; % no reconstruction.
    Event(event_Count).process = 0; % no processing
...
filename = 'Cardiac_Cycle_Mat.mat';
save(filename);
VSX
