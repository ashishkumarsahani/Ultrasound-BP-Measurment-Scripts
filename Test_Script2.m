...
pressure_In_Tube = 95;
voltage_Levels = [50:-2:5];
current_Voltage_Level_Index = 1;
focal_Length = 54.5; %in wavelength
...
Resource.Parameters.simulateMode = 0;  % Run in simulation mode
...
Resource.VDAS.dmaTimeout = 50000;
...
time_us_pushend2imagestart =  100;
...
Event(event_Count-1).seqControl = [seq_Count seq_Count+1]; % PRT delay
...
for i = (l-1)*num_of_Frames_Per_Push_Cycle+1:l*num_of_Frames_Per_Push_Cycle
        ...
        Event(event_Count).process = 6; % no processing
...
Process(6).method = 'reconstruction_Initiated';
