...
NumFrameStore = 1;
NumTrackingFrames = 10;

StartDepthWL = 60;
EndDepthWL = 140;
PushFocusWL = 90;
PulseRepPeriod = 10000; % Frame Period in milliseconds;
PushSupplyChargeTime = 5; % Initial Delay for HV supply to Charge up (in ms)
...
PData(1).Origin = [-Trans.spacing*63.5,0,SFormat(1).startDepth]; % x,y,z of uppr lft crnr.
...
TPC(3).name = 'Doppler';
TPC(3).maxHighVoltage = 35;
TPC(5).name = 'Push';
TPC(5).maxHighVoltage = 45;
...
SeqControl(4).argument =  5000; % time in usec 
...
for i = 1:NumFrameStore
 ...
 for qwe = 1:push_iteration
    ...
    Event(n-1).seqControl = 2;  % Reduce delay after last push-pulse
...
%     % Wait for Delay to Transfer Data
%         SeqControl(nsc).command = 'noop';%
%         SeqControl(nsc).argument =  50000; % time in usec 
%         Event(n).info = 'Delay to transfer Data';
%         Event(n).tx = 0;
%         Event(n).rcv = 0;
%         Event(n).recon = 0;
%         Event(n).process = 0;
