...
NumFrameStore = 80;
NumTrackingFrames = 1;
PixelX_WL = 0.05;  % Was 0.05 Set reconstruction lateral grid size (in wavelengths)
PixelZ_WL = 0.05;  % Was 0.05 Set reconstruction axial grid size (in wavelengths)

StartDepthWL = 57;  % was 70
EndDepthWL = 60;   % was 110
WidthPercent = 10;  % was 20; Width of the aperture to use for imaging (scale to <100% to reduce image width).
PushFocusWL = 58;  % was 80

PulseRepPeriod = 2000; % Frame Period in milliseconds;
PushSupplyChargeTime = 15; % Initial Delay for HV supply to Charge up (in ms)

% Push and Tracking Timing Parameters 
push_iteration = 4;  % Creates multiple bursts of 64 x Nslider
time_us_between_push = 10; 
time_us_pushend2imagestart = 50; % 20us minimum to avoid timing error; 20-100 will have small artifacts
trackPRF =12000; % PRF for tracking images; >12k generates timing error
...
% Specify Media object. %---Not necessary
pt1;
...
Resource.RcvBuffer(1).rowsPerFrame = ceil(sqrt(SFormat(1).endDepth^2 + (Trans.numelements*Trans.spacing)^2))*8*(1+ne);%2048*(1 + ne); % 1 ref acqusition + ne tracking acqusitions 
...
Resource.DisplayWindow(1).pdelta = 0.05;
...
TW_push_freq_MHz = 5;
[parametric_A_push, ~,TW_push_freq_MHz ] = TW_parameter_generator( TW_push_freq_MHz ); %---Probably not needed. Can be taken directly from table.

TW(3).type = 'parametric';
TW(3).Parameters = [parametric_A_push,17,push_pulse_halfcyc_number,1];   % A, B, C, D %---push_pulse_halfcyc_number =31. In extended burst this would become x64!
TW(3).extendBL = 1;

push_pulse_duration_us = (push_pulse_halfcyc_number/2)/TW_push_freq_MHz *64 ; %  TW(3).extendBL = 1; =>64 times  %---Seems like the above push pulse length is intentional
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Receive(k+1).callMediaFunc = 1; %---This is not required. Only used in simulate mode.
...
Process(6).method = 'change_Colormap';
...
n = 1; % Variable to keep track of event number.
    ...
    for i = 1:NumFrameStore            
        ...
        Event(n).info = 'Run Sonetics_ExtScript';
        Event(n).tx = 0;
        Event(n).rcv = 0;
        Event(n).process = 4;
        Event(n).seqControl = nsc;
        ...
        Event(n).info = 'change display';
        Event(n).tx = 0;         % no transmit
        Event(n).rcv = 0;        % no rcv
        Event(n).recon = 0;  % reconstruction for both 2D
        Event(n).process = 6;    % process 2D
        Event(n).seqControl = nsc;
        n = n + 1;
