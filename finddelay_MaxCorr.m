function [d,max_c]  = finddelay_MaxCorr(x,y,varargin)
...
if ~isnumeric(x)
    % x must be numeric.
    error(message('comm:finddelay:firstInputNumeric'));
elseif (ndims(x)>2) || (isempty(x))
    % x must be a vector or a matrix.
    error(message('comm:finddelay:firstInputVectorMatrix'));
end
...
if ~isnumeric(y),
    % y must be numeric.
    error(message('comm:finddelay:secondInputNumeric'));
elseif (ndims(y)>2) || (isempty(y)),
    % y must be a vector or a matrix.
    error(message('comm:finddelay:secondInputVectorMatrix'));
end
% fft function in xcorr only works on double and single data types.
y = double(y);    
if isvector(y)
...
if ~isvector(x) && ~isvector(y)
    if nx~=ny
        error(message('comm:finddelay:sameNumberColumns'));
    end    
end    
